		...
		try{
			...
			for (Permohonan p1 : listParent) {
				...
				for(Permohonandetail p2 : listChildren){
					if(p2.getPermohonan().getPermohonanId().equals(p1.getPermohonanId()) && p2.getBiodata().getKelurahan().getKecamatan().getKecamatanId().equals(kecamatanID)){
						jsonChild = new JSONObject();
						jsonChild.put("permohonanID", "x"+p2.getPermohonanDetailId());
